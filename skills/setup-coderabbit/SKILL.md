...
6. **Enable matching `tools`.** Only turn on what the stack uses (e.g. Python→ruff, JS/TS→eslint+biome, Terraform→tflint+checkov, GitHub Actions→actionlint). Always enable `gitleaks`. Leave everything else at defaults.
